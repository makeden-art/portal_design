...
def _platform_catalog() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "role": "module",
            "icon": "📐",
            "name": "lisp_Nikolay — инженерные утилиты",
            "tagline": "Модуль портала (плагин)",
            "description": (
                "Поперечники, нормоконтроль, проверка ЭЦП, конвейер PDF. "
                "Подключается к порталу как PORTAL_PLUGINS=portal_utilities."
            ),
            "features_dynamic": True,
            "publishable": True,
            "required": False,
            "compose_service": None,
            "install_hint": "Входит в образ портала; отдельный репозиторий makeden-art/lisp_Nikolay",
        },
        {
            "id": "masha-print",
            "role": "addon",
        ...
        },
        {
            ...
            "role": "local",
            "icon": "🔗",
            "name": "n8n (оркестрация)",
            "tagline": "Только на вашем сервере — не в публичном портале",
            "description": "Webhook-цепочки, уведомления, связка PDF ↔ задачи.",
            "features": ["Автоматизация бизнес-процессов", "Webhook для PDF-конвейера"],
            "publishable": False,
            "required": False,
            "compose_service": "n8n",
            "health_url": os.getenv("N8N_HEALTH_URL", "http://n8n:5678/healthz"),
            "public_url": os.getenv("N8N_PUBLIC_URL", "http://192.168.88.10:5678/"),
            "install_hint": f"{compose} up -d n8n",
        },
        {
            "id": "openproject",
            "role": "local",
            ...
            "name": "OpenProject",
            "tagline": "Только на вашем сервере — не в публичном портале",
            "description": "Проекты, задачи, сроки, Gantt для заказчика.",
            "features": ["Work packages", "Сроки и исполнители"],
            "publishable": False,
            "required": False,
            "compose_service": None,
            "health_url": os.getenv("OPENPROJECT_URL", "http://192.168.88.10:8081").rstrip("/") + "/",
            "public_url": os.getenv("OPENPROJECT_URL", "http://192.168.88.10:8081/"),
            "install_hint": f"cd {PLATFORM_ROOT}/openproject-official && docker compose up -d",
        },
    ]
